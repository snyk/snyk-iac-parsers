package source_line_emitter

//Copied from https://github.com/tmccombs/hcl2json/releases/tag/v0.3.1 and added json annotations
...
func (c *converter) convertBody(body *hclsyntax.Body) (jsonObj, error) {
	var err error
	out := make(jsonObj)
	addLineInfoForBody(out, body)
	for key, value := range body.Attributes {
		out[key], err = c.convertExpression(value.Expr)

		addLineInfoForAttribute(out, key, value)
		if err != nil {
			return nil, err
		}
	}
...
func addLineInfoForAttribute(out jsonObj, key string, value *hclsyntax.Attribute) {
	out[fmt.Sprintf("_snyk_att_src_%s", key)] = value.SrcRange
}
func addLineInfoForBody(out jsonObj, body *hclsyntax.Body) {
	out["_snyk_body_src"] = body.SrcRange
}
